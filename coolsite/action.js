...
var event_tap = function(_act){
    coolsite360.$(_act['_obj']).on('tap',function(e){
        console.log('TAP OK!');
        _execType(_act);
    })
}
...
var _changeState = function(detail){
    ...
    if(_arguments.e_ids && _arguments.cla){
        ...
        for(var i =0;i<_arguments.e_ids.length;i++){
            if(_class == 'c-state1')
                coolsite360.$(_arguments.e_ids[i]).addClass(_class).removeClass('c-state2','c-state3');
            else if(_class == 'c-state2')
                coolsite360.$(_arguments.e_ids[i]).addClass(_class).removeClass('c-state1','c-state3');
            else  coolsite360.$(_arguments.e_ids[i]).addClass(_class).removeClass('c-state1','c-state2');
        }
    }else{
